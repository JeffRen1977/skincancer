"""
Image2 CNN for skin cancer classification (PyTorch).
160x160 input, ReLU, no dropout. Data: one subfolder per class.

Run training:  python image2_cnn_torch.py
Run analysis:  python image2_cnn_torch.py --analyze
"""
...
class Model(torch.nn.Module):
    """CNN: conv 9/5/3 -> flatten -> linear (no dropout) -> 7 logits."""

    def __init__(self, input_shape):
        super().__init__()
        print(f"{'Input Shape:':>30}", input_shape)
        self.zp1 = torch.nn.ZeroPad2d((1, 1, 1, 1))
        self.conv1 = torch.nn.Conv2d(3, 24, kernel_size=9, stride=3)
        self.relu = torch.nn.ReLU()
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=3, stride=2)
        self.conv2 = torch.nn.Conv2d(24, 48, kernel_size=5, stride=2)
...
def analyze_class_accuracy(
    model_path="saves/image2_cnn_model.pth",
    data_dir=None,
    max_images_per_class=50,
    output_path="saves/class_accuracy_image2_cnn.png",
    device=None,
):
    ...
    print("\n" + "=" * 80)
    print("CLASS-WISE ACCURACY (Image2 CNN)")
    print("=" * 80)
    ...
    ax.set_title('Per-Class Accuracy (Image2 CNN)')
    ax.set_xlim([0, 100])
...
def main():
    parser = argparse.ArgumentParser(description="Image2 CNN: train or analyze")
    parser.add_argument("--analyze", action="store_true")
    parser.add_argument("--model", default="saves/image2_cnn_model.pth")
    parser.add_argument("--data_dir", default=None)
    parser.add_argument("--max_images", type=int, default=50)
    parser.add_argument(
        "--output", default="saves/class_accuracy_image2_cnn.png"
    )
    ...
    for e in range(NUM_EPOCHS):
        ...
        print(
            f"Epoch {e + 1}/{NUM_EPOCHS}  train_loss={train_loss:.4f}  "
    ...
    plt.tight_layout()
    plt.savefig(
        "saves/training_history_image2_cnn.png",
        dpi=300, bbox_inches="tight"
    )
    plt.close()
    print("Training history saved: saves/training_history_image2_cnn.png")

    torch.save(model.state_dict(), "saves/image2_cnn_model.pth")
    print("Model saved: saves/image2_cnn_model.pth")
