...
Run analysis:  python second_cnn_torch.py --analyze --model saves/second_cnn_model.pth --data_dir skincancer/organized --max_images 50 --output saves/class_accuracy_second_cnn.png
"""
...
# -----------------------------------------------------------------------------
# Class accuracy analysis (second_cnn model only)
# -----------------------------------------------------------------------------
...
def _preprocess_image_second_cnn(image_path):
    """Load and preprocess image for second_cnn (256x256, [0,1] range). Returns tensor or None."""
...
def analyze_class_accuracy(model_path="saves/second_cnn_model.pth", data_dir=None,
                           max_images_per_class=50, output_path="saves/class_accuracy_second_cnn.png",
                           device=None):
    """
    Analyze per-class accuracy for the second_cnn model.
    ...
    print("\n" + "=" * 80)
    print("CLASS-WISE ACCURACY ANALYSIS (Second CNN)")
    print("=" * 80)
    ...
    for class_idx in range(len(display_names)):
        ...
        for image_path in images:
            tensor = _preprocess_image_second_cnn(image_path)
            if tensor is None:
                continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Second CNN: train or analyze class accuracy")
    parser.add_argument("--analyze", action="store_true", help="Run class accuracy analysis instead of training")
    parser.add_argument("--model", default="saves/second_cnn_model.pth", help="Model path (for --analyze)")
    ...
    parser.add_argument("--output", default="saves/class_accuracy_second_cnn.png", help="Output plot path (for --analyze)")
