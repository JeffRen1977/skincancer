...
Configurable activation (LeakyReLU default). Data: one subfolder per class.

Run training:  python second_cnn_torch.py
Run analysis:  python second_cnn_torch.py --analyze
"""
...
def get_activation(name: str) -> torch.nn.Module:
    ...
    raise ValueError(f"Unknown: {name}. Use: relu, leaky_relu, gelu, silu, elu")
...
class Dataset(torch.utils.data.Dataset):
    """Load images from class folders. CPU for augmentation."""
...
class Model(torch.nn.Module):
    """CNN with configurable activation. Same layout as first_cnn."""

    def __init__(self, input_shape, activation_name: str = ACTIVATION):
        super().__init__()
...
def get_dataloaders(
    dataset, train_prop, batch_size, n_classes, train_transform=None
):
    """Split train/val, wrap train with aug, WeightedRandomSampler 4x."""
    ...
    train_wrap = TransformWrapper(train_set, transform=train_transform)
    train_ld = torch.utils.data.DataLoader(
        train_wrap, batch_size=batch_size, sampler=sampler
    )
...
def analyze_class_accuracy(
    model_path="saves/second_cnn_model.pth",
    data_dir=None,
    max_images_per_class=50,
    output_path="saves/class_accuracy_second_cnn.png",
    device=None,
):
    ...
    print("\n" + "=" * 80)
    print("CLASS-WISE ACCURACY (Second CNN)")
    print("=" * 80)
    ...
    ax.set_title('Per-Class Accuracy (Second CNN)')
    ax.set_xlim([0, 100])
...
def main():
    parser = argparse.ArgumentParser(description="Second CNN: train or analyze")
    parser.add_argument("--analyze", action="store_true")
    parser.add_argument("--model", default="saves/second_cnn_model.pth")
    parser.add_argument("--data_dir", default=None)
    parser.add_argument("--max_images", type=int, default=50)
    parser.add_argument(
        "--output", default="saves/class_accuracy_second_cnn.png"
    )
    ...
    out = open("saves/printout_second_" + TIME_STAMP + ".txt", "a")
    ...
    plt.tight_layout()
    plt.savefig(
        "saves/training_history_second_cnn.png",
        dpi=300, bbox_inches="tight"
    )
    plt.close()
    print("Training history saved: saves/training_history_second_cnn.png")

    torch.save(model.state_dict(), "saves/second_cnn_model.pth")
    print("Model saved: saves/second_cnn_model.pth")
